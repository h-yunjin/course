...
def get_hotels(page: int | None = Query(None, description="количество отелей на странице"),
               per_page: int | None = Query(None, description="номер страницы"),
        ...
        if page and per_page:
            ...
            result = hotels_[offset - page: offset]
        if page and not per_page:
            result = hotels_[0: page] 
        if not page and per_page:
            offset = 4 * per_page
            result = hotels_[offset - 4: offset]           
        return result
