...
class FacilitiesRoomRepositories(BaseRepositories):
    ...
    async def edit_servises(self, room_id: int, servises_ids: list[int]) -> None:
        query = select(FacilitiesRoomsOrm.servise_id).filter(
            FacilitiesRoomsOrm.room_id == room_id
        )
        ...
        if del_execute:
            del_query = delete(FacilitiesRoomsOrm).filter(
                FacilitiesRoomsOrm.room_id == room_id,
                FacilitiesRoomsOrm.servise_id.in_(del_execute),
            )
            await self.session.execute(del_query)

        if insert_execute:
            insert_query = insert(FacilitiesRoomsOrm).values(
                [{"room_id": room_id, "servise_id": sr_id} for sr_id in insert_execute]
            )
